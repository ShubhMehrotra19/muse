...
function Page(props: Props) {
    ...
    const updateInputState = (inputName: string, value: string, isFocused: boolean) => {
        ...
        <section className='mx-5 overflow-hidden'>
            <section className='flex justify-between items-end'>
                ...
                <div className='flex flex-col px-5 py-2 items-center h-[84vh] w-[30%]'>
                    <div className=' border-2 border-slate-300 shadow-md rounded-md flex flex-col justify-center items-center px-4 py-1 h-full w-full'>
                        ...
                        <div className='flex flex-col justify-center items-start w-full relative mb-5'>
                            <p className={`text-xs font-semibold text-slate-400 bg-white px-1 py-[2px] ${inputState.name.isFocused || inputState.name.value ? 'absolute left-2 -top-2' : 'absolute left-2'}`}>Name</p>
                            <input
                                onFocus={() => updateInputState('name', inputState.name.value, true)}
                                onBlur={() => updateInputState('name', inputState.name.value, false)}
                                onChange={(e) => updateInputState('name', e.target.value, inputState.name.isFocused)}
                        ...
                        <div className='flex flex-col justify-center items-start w-full relative mb-5'>
                            <p className={`text-xs font-semibold text-slate-400 bg-white px-1 py-[2px] ${inputState.phoneNumber.isFocused || inputState.phoneNumber.value ? 'absolute left-2 -top-2' : 'absolute left-2'}`}>Phone Number</p>
                            <input
                                onFocus={() => updateInputState('phoneNumber', inputState.phoneNumber.value, true)}
                                onBlur={() => updateInputState('phoneNumber', inputState.phoneNumber.value, false)}
                                onChange={(e) => updateInputState('phoneNumber', e.target.value, inputState.phoneNumber.isFocused)}
                                value={inputState.phoneNumber.value}
                                className='py-2 pl-2 w-full rounded-md border-2 border-slate-500 font-medium text-base text-stone-600/90'
                            />
                        </div>
                        <div className='flex flex-col justify-center items-start w-full relative mb-5'>
                            <p className={`text-xs font-semibold text-slate-400 bg-white px-1 py-[2px] ${inputState.email.isFocused || inputState.email.value ? 'absolute left-2 -top-2' : 'absolute left-2'}`}>Email</p>
                            <input
                                onFocus={() => updateInputState('email', inputState.email.value, true)}
                                onBlur={() => updateInputState('email', inputState.email.value, false)}
                                onChange={(e) => updateInputState('email', e.target.value, inputState.email.isFocused)}
                                value={inputState.email.value}
                                className='py-2 pl-2 w-full rounded-md border-2 border-slate-500 font-medium text-base text-stone-600/90'
                            />
                        </div>
                        <div className='flex flex-col justify-center items-start w-full relative mb-5'>
                            <p className={`text-xs font-semibold text-slate-400 bg-white px-1 py-[2px] ${inputState.password.isFocused || inputState.password.value ? 'absolute left-2 -top-2' : 'absolute left-2'}`}>Password</p>
                            <input
                                onFocus={() => updateInputState('password', inputState.password.value, true)}
                                onBlur={() => updateInputState('password', inputState.password.value, false)}
                                onChange={(e) => updateInputState('password', e.target.value, inputState.password.isFocused)}
                ...
                <div className='flex flex-col h-[84vh] px-5 py-2 w-[40%]'>
                    ...
                    <div className='border-2 border-slate-300 shadow-md rounded-md flex flex-col px-4 py-3 h-full w-full overflow-y-scroll' style={{
                        scrollbarWidth: 'none'
                    }}>
                        <PostComponent />
                        <PostComponent />
